...
<div>
    ...
    <ul class="nav nav-tabs">
        <shiro:hasPermission name="customer:upload:create">
        <c:if test="${op eq '新增'}">
            <li ${op eq '新增' ? 'class="active"' : ''}>
                <a href="${ctx}/customer/upload/create?BackURL=<es:BackURL/>">
        ...
        <c:if test="${not empty m.id}">
            <li ${op eq '查看' ? 'class="active"' : ''}>
                <a href="${ctx}/customer/upload/${m.id}?BackURL=<es:BackURL/>">
                    <i class="icon-eye-open"></i>
                    查看
                </a>
            </li>
            <shiro:hasPermission name="customer:upload:update">
            <li ${op eq '修改' ? 'class="active"' : ''}>
                <a href="${ctx}/customer/upload/${m.id}/update?BackURL=<es:BackURL/>">
                    <i class="icon-edit"></i>
                    修改
                </a>
            </li>
            </shiro:hasPermission>
            <shiro:hasPermission name="customer:upload:delete">
            <li ${op eq '删除' ? 'class="active"' : ''}>
                <a href="${ctx}/customer/upload/${m.id}/delete?BackURL=<es:BackURL/>">
    ...
    <form:form id="editForm" method="post" commandName="m" cssClass="form-horizontal" enctype="multipart/form-data">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <form:input path="name"  cssClass="validate[required,maxSize[20]]" placeholder="最多20个字符"/>
                </div>
            </div>
